...
<img src="https://img.alicdn.com/tfs/TB1XOX43rr1gK0jSZFDXXb9yVXa-800-400.png" width="200" alt="pblint Logo" />

pblint 是《滴滴IBG外卖前端规约》的配套 Lint 工具，可以为项目一键接入规约、一键扫描和修复规约问题，保障项目的编码规范和代码质量。

## 背景

我们引入了多个业界流行的 Linter 作为《滴滴IBG外卖前端规约》的配套，并根据规约内容定制了规则包，它们包括：
...
pblint 收敛屏蔽了这些依赖和配置细节，提供简单的 CLI 和 Node.js API，让项目能够一键接入、一键扫描、一键修复、一键升级，并为项目配置 git commit 卡口，降低项目接入规约的成本。
...
```bash
npm install pblint -g
```

安装完成后，可执行 `pblint -h` 以验证安装成功。

### 使用

#### `pblint init`：一键接入

在项目根目录执行 `pblint init`，即可一键接入规约，为项目安装规约 Lint 所需的依赖和配置。

![pblint init](https://img.alicdn.com/tfs/TB1jJuDV.H1gK0jSZSyXXXtlpXa-1897-778.png)
...
- 写入各种配置文件，包括：
  ...
  - `pblint.config.js`：pblint 包的一些配置，如启用的功能等
- 配置 git commit 卡口：使用 [husky](https://www.npmjs.com/package/husky) 设置代码提交卡口，在 git commit 时会运行 `pblint commit-file-scan` 和 `pblint commit-msg-scan` 分别对提交文件和提交信息进行规约检查。`pblint commit-file-scan` 默认仅对 error 问题卡口，如果你想对 warn 问题也卡口，可以增加 `--strict` 参数以开启严格模式

> 注 1：如果项目已经配置过 ESLint、stylelint 等 Linter，执行 `pblint init` 将会提示存在冲突的依赖和配置，并在得到确认后进行覆盖：
> ![conflict resolve](https://img.alicdn.com/tfs/TB1HWvjhsieb18jSZFvXXaI3FXa-1538-749.png)
>
> 注 2：如果项目的 .vscode/ 目录被 .gitignore 忽略，可以在拉取项目后单独执行 `pblint init --vscode` 命令写入 `.vscode/extensions.json` 和 `.vscode/settings.json` 配置文件

#### `pblint scan`：一键扫描

在项目的根目录执行命令，即可扫描项目的规约问题：

![pblint scan](https://img.alicdn.com/tfs/TB1XWlolLzO3e4jSZFxXXaP_FXa-1268-305.png)
...
> 注 1：事实上，你可以在任意目录执行 `pblint scan`，pblint 会根据文件类型、JSON 等特征嗅探项目类型。但我们还是推荐在执行过 `pblint init` 的项目根目录执行 `pblint scan`，以得到最准确的扫描结果。
>
> 注 2：pblint 会根据项目内有无 eslint 和 stylelint 配置文件判断使用项目的配置文件还是 pblint 默认配置进行扫描。若使用项目的，在未安装依赖时会帮其安装（执行 npm i）。若使用项目配置扫描失败，则使用默认配置扫描

#### `pblint fix`：一键修复

在项目的根目录执行命令，即可修复部分规约问题：

![pblint fix](https://img.alicdn.com/tfs/TB1DOOni7cx_u4jSZFlXXXnUFXa-924-317.png)
...
#### `pblint commit-file-scan` 提交文件扫描
...
- pblint.init(config)：将项目一键接入规约，效果等同于 `pblint init`
...
| -------- | -------- | -------- | -------- |
| cwd | string | - | 项目绝对路径 |
| eslintType | ESLintType | - | 语言和框架类型，如果不配置，等同于 pblint init，控制台会出现选择器，如果配置，控制台就不会出现选择器 |
| enableESLint | boolean | true | 是否启用 ESLint，如果不配置默认值为 true，即默认启用 ESLint |
| enableStylelint | boolean | - | 是否启用 stylelint，如果不配置，等同于 pblint init，控制台会出现选择器，如果配置，控制台就不会出现选择器 |
| enableMarkdownlint | boolean | - | 是否启用 markdownlint，如果不配置，等同于 pblint init，控制台会出现选择器，如果配置，控制台就不会出现选择器 |
...
- pblint.scan(config): 扫描当前项目代码，效果等同 `pblint scan`
...
| config | [Config](https://github.com/alibaba/f2e-spec/blob/main/packages/pblint/src/types.ts#L16) | - | 指定要运行的 linter，优先级高于 pblint.config.js，不传该参数则默认读取 pblint.config.js 作为 config |

## 配置

pblint 基于一份配置进行扫描（但你也可以零配置使用），支持的配置参数有：
...
pblint 会读取执行目录下的 `pblint.config.js` 作为配置文件。`pblint init` 会在执行目录下新增如下的 `pblint.config.js` 文件：
...
如果你的 TS 项目 commit 卡口和 `pblint scan` 运行时间很长，可以通过如下在 .eslintrc.js 中增加以下配置提升性能：
